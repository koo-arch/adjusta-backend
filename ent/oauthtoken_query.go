...
func (otq *OAuthTokenQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (otq *OAuthTokenQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (otq *OAuthTokenQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (otq *OAuthTokenQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := otq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (otq *OAuthTokenQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*OAuthToken, init func(*OAuthToken), assign func(*OAuthToken, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_oauth_token == nil {
			continue
		}
		fk := *nodes[i].user_oauth_token
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_oauth_token" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
